...
class RainfallAndSewerPipeInfoApiView(APIView):
    ...
    def get(self, request, gubn, datetime_info):
        ...
        if len(datetime_info) != 12:
            return Response(
                {"error": "datetime_info 값을 확인해주세요. datetime_info = YYYYMMDDHHmm"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
        ...
        return Response(
            GuNameModelSerializer(gu_name, context={"datetime": str_to_datetime}).data, status=status.HTTP_200_OK
        )
